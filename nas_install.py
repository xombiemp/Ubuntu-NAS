...
sudo apt-get install git-core subversion build-essential automake libtool libcppunit-dev libcurl4-openssl-dev libsigc++-2.0-dev unzip unrar curl libncurses-dev ffmpeg -y
...
libtorrent_git="git://github.com/rakshasa/libtorrent.git"
rtorrent_git="git://github.com/rakshasa/rtorrent.git"
...
cd $libtorrent_path
oldgit=$(git describe --always)
service rtorrentd stop
git pull
newgit=$(git describe --always)
if [ "$newgit" != "$oldgit" ] ; then
  ./autogen.sh
  ./configure
  make
  make install
  echo -e "libtorrent updated from $oldgit to $newgit\\n"
  sleep 2
  updated="yes"
else
  echo -e "libtorrent is already at the latest version: $newgit\\n"
  sleep 2
fi
...
git pull
newgit=$(git describe --always)
if [ "$newgit" != "$oldgit" ] || [ "$updated" == "yes" ] ; then
  ./autogen.sh
  ...
  ldconfig
  if [ "$newgit" != "$oldgit" ] ; then
    echo -e "rtorrent updated from $oldgit to $newgit\\n"
    sleep 2
  else
    echo -e "rtorrent was recompiled and is at version $newgit\\n"
    sleep 2
  fi
  updated="yes"
else
  echo -e "rtorrent is already at the latest version: $newgit\\n"
  sleep 2
fi
